<template>
	<v-app-bar color="primary-darken-1">
		...
		<v-app-bar-title @click="router.push({ name: 'home' })" style="cursor: pointer"
			>Thunderous Knockout Fighting</v-app-bar-title
		>
		...
		<template v-if="!isConnected">
			<router-link to="/bets">
				<v-btn color="white"
					><v-icon class="mr-2">mdi-checkbox-marked</v-icon>My bets</v-btn
				>
			</router-link>
			<v-menu>
				<template v-slot:activator="{ props }">
					<v-btn v-bind="props" color="white" class="mr-3"
						><v-icon size="30" class="mr-2">mdi-account</v-icon>Simon</v-btn
					>
				</template>
		...
		<template v-else>
			<div class="d-flex mr-3" style="gap: 0.75rem">
				<v-btn @click="router.push({ name: 'signup' })" color="secondary" variant="flat"
					>signup
				</v-btn>
				<v-btn @click="router.push({ name: 'login' })" color="white" variant="flat"
					>signin
				</v-btn>
			</div>
		</template>
