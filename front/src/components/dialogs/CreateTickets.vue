                ...
                <v-container>
                    <v-form v-model="valid" ref="form">
                        <v-row justify="space-between" class="align-center">
                            <v-col>
                                <v-text-field v-model.number="ticket.price" type="number" min="1" :rules="[rules.required, rules.ticketPrice]" placeholder="0" label="Ticket Price" />
                            </v-col>
                            <v-col>
                                <v-select v-model="ticket.ticketCategory" item-value="name" :items="ticketCategoriesName" :rules="[rules.required]" placeholder="Categories" label="Ticket category" >
                                </v-select>
                            </v-col>
                            <v-col>
                                <v-text-field
                                    v-model="numberOfTickets"
                                    :rules="[rules.required, rules.ticketPlaces]"
                                    type="number"
                                    min="1"
                                    placeholder="1"
...
import { ITicket, ICreateTicket } from '@/service/api/tickets';
...
export default defineComponent({
    setup() {
        ...
        const ticket = reactive<ICreateTicket>({
            price: 1,
            event: eventId.value,
            availability: true,
            ticketCategory: ''
        ...
        const submit = async () => {
            try {
                ...
                if (valid) {
                    const { price, availability } = ticket;

                    const payload: ICreateTicket = {
                        price,
                        availability,
